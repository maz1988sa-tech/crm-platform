...
(function (root) {
  ...
  TABS.users = function (pane) {
    ...
    pane.appendChild(UI.card({ title: t('ad.permissions_matrix'), sub: S.lang === 'en' ? 'Source: src/config/permissions.js — enforced by the backend (RLS/RPC) and the local adapter' : 'المصدر: src/config/permissions.js — تُفرض في الخلفية (RLS/RPC) وفي المحوّل المحلي', tight: true, body: UI.table({ columns: mcols, rows: PC.permissions, responsive: false }) }));
  };
  ...
  TABS.config = function (pane) {
    ...
    var sel = h('select', { class: 'sel' }); lists.forEach(function (k) { sel.appendChild(h('option', { value: k }, k)); });
    var listBox = h('div');
    function renderList() {
      D.clear(listBox);
      ...
      listBox.appendChild(UI.card({ title: key, sub: items.length + ' ' + (S.lang === 'en' ? 'items' : 'عنصر'), tight: true, actions: canEdit ? h('button', { class: 'btn sm', type: 'button', on: { click: function () { itemForm(null); } } }, UI.icon('plus'), t('ad.config_add')) : null,
      ...
      function itemForm(it) {
        UI.formModal({ title: it ? t('app.edit') : t('ad.config_add'), sub: key, size: 'sm', cols: 1, fields: [{ key: 'key', label: t('ad.config_key'), type: 'text', required: true, disabled: !!it, dir: 'ltr' }, { key: 'ar', label: t('ad.config_ar'), type: 'text', required: true }, { key: 'en', label: t('ad.config_en'), type: 'text', dir: 'ltr' }, { key: 'color', label: S.lang === 'en' ? 'Colour' : 'اللون', type: 'select', options: ['slate', 'ok', 'warn', 'danger', 'info', 'accent', 'ink'].map(function (c) { return { value: c, label: c }; }) }], values: it ? U.clone(it) : {},
    ...
    /* العتبات */
    var thr = [
      ['rules.hygiene.no_activity_days', RULES.hygiene.no_activity_days, S.lang === 'en' ? 'Days without activity before flagging' : 'أيام بلا نشاط قبل التعليم'],
      ...
      ['rules.hygiene.customer_no_interaction_days', RULES.hygiene.customer_no_interaction_days, S.lang === 'en' ? 'Customer no-interaction days' : 'أيام بلا تفاعل للعميل'],
      ['rules.reminders.escalate_after_days', RULES.reminders.escalate_after_days, S.lang === 'en' ? 'Escalate overdue high-priority after (days)' : 'تصعيد المتأخر عالي الأولوية بعد (يوم)'],
  ...
  TABS.duplicates = function (pane) {
    ...
    pane.appendChild(UI.infoBox(S.lang === 'en' ? 'Detection uses CR / unified / VAT numbers, normalised name similarity, email and phone. Nothing is merged automatically; every merge or dismissal is audited.' : 'يعتمد الكشف على السجل التجاري/الرقم الموحد/الرقم الضريبي، وتشابه الاسم بعد التطبيع، والبريد والهاتف. لا دمج تلقائي؛ كل دمج أو استبعاد يُسجَّل في التدقيق.'));
    function custRow(p) {
    ...
    pane.appendChild(UI.card({ title: t('nav.customers'), sub: cd.length + ' ' + (S.lang === 'en' ? 'suspected pairs' : 'زوج مشتبه'), tight: true, body: cd.length ? UI.list(cd, custRow) : UI.empty(t('ad.dup_none'), ' ') }));
    pane.appendChild(h('div', { style: { height: '12px' } }));
    pane.appendChild(UI.card({ title: t('cu.tab_contacts'), sub: kd.length + ' ' + (S.lang === 'en' ? 'suspected pairs' : 'زوج مشتبه'), tight: true, body: kd.length ? UI.list(kd, function (p) { return h('div', null, h('span', { class: 'avatar warn' }, '?'), h('div', { class: 'main' }, h('div', { class: 't' }, p.a.full_name + ' ⟷ ' + p.b.full_name), h('div', { class: 's' }, S.customerName(p.a.customer_id) + ' / ' + S.customerName(p.b.customer_id) + ' — ' + p.reasons.map(function (r) { return t('ad.dup_reason_' + r); }).join('، '))), h('div', { class: 'end' }, h('button', { class: 'btn xs ghost', type: 'button', on: { click: function () { S.adapter.dismissDuplicate('contact', p.a.id, p.b.id).then(function () { return root.APP.rerender(); }).catch(UI.errorToast); } } }, t('ad.dup_dismiss')))); }) : UI.empty(t('ad.dup_none'), ' ') }));
  ...
  TABS.import = function (pane) {
    ...
    function showResult(res, fileName) {
      ...
      if (res.unknown_columns && res.unknown_columns.length) result.appendChild(UI.warnBox((S.lang === 'en' ? 'Ignored unknown columns: ' : 'أعمدة غير معروفة تم تجاهلها: ') + res.unknown_columns.join('، ')));
  ...
  TABS.integration = function (pane) {
    ...
    S.adapter.health().then(function (hh) {
      ...
      health.appendChild(h('div', { class: 'grid c2' },
        ...
        UI.card({ title: S.lang === 'en' ? 'Integration boundary' : 'حدود التكامل', body: h('div', { class: 'small' }, h('p', null, S.lang === 'en' ? '• Master data from the data centre carries source_system / source_id / synced_at and origin=datacentre.' : '• البيانات الرئيسية من مركز البيانات تحمل source_system / source_id / synced_at و origin=datacentre.'), h('p', null, S.lang === 'en' ? '• Platform-entered data: origin=platform; imported files: origin=import; seed: origin=demo.' : '• البيانات المُدخلة في المنصة origin=platform؛ المستوردة من ملف origin=import؛ الأولية origin=demo.'), h('p', null, S.lang === 'en' ? '• Derived values (weighted value, days in stage, flags) are never stored.' : '• القيم المشتقة (القيمة المرجحة، أيام المرحلة، الأعلام) لا تُخزَّن أبدًا.'), h('p', null, S.lang === 'en' ? '• Sync conflicts: a data-centre record never overwrites a newer platform edit silently; conflicts are queued for review (see docs/DATA-MODEL.md).' : '• تعارضات المزامنة: لا يستبدل سجل مركز البيانات تعديلًا أحدث في المنصة تلقائيًا؛ التعارضات تُعرض للمراجعة (انظر docs/DATA-MODEL.md).')) }),
        S.can('admin.config') && S.adapter.purgeDemo ? UI.card({ title: t('app.demo_purge'), body: h('div', null,
          h('p', { class: 'small muted' }, S.mode === 'supabase'
            ? (S.lang === 'en' ? 'Removes every seed record (origin=demo) from the Supabase project, deactivates the preloaded profiles and deletes their Auth accounts when another active system administrator exists. Irreversible.' : 'يحذف كل السجلات الأولية (origin=demo) من مشروع Supabase، ويوقف الملفات المحمّلة مسبقًا ويحذف حساباتها من Auth عند وجود مسؤول نظام آخر نشط. لا يمكن التراجع.')
            : (S.lang === 'en' ? 'Removes every seed record from this browser store. Irreversible.' : 'يحذف كل السجلات الأولية من مخزن هذا المتصفح. لا يمكن التراجع.')),
          hh.demoStatus ? h('p', { class: 'small' }, (S.lang === 'en' ? 'Seed records on the server: ' : 'السجلات الأولية على الخادم: ') + U.fmtNum(hh.demoStatus.total) + (S.lang === 'en' ? ' · preloaded users: ' : ' · مستخدمون محمّلون مسبقًا: ') + U.fmtNum(hh.demoStatus.users)) : null,
